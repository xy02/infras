package com.github.xy02.infras

import com.github.xy02.ritp_kt.*
import com.github.xy02.ritp_kt.Context
...
//    val init = initWith(myInfo)
    ...
    ctxs.flatMap { ctx -> onContext(ctx) }.subscribe()
...
private fun onContext(cx: Context): Observable<Int> {
    val conn = cx.connection
    val failed = false //模拟处理成功
    val firstPull = if (failed)
        Observable.error(Exception("failed")) else
        Observable.just(10000).doOnNext { registerFns(cx) } //注册功能
...
private fun registerFns(cx: Context) {
    Observable.merge(
        accFn(cx),  //其他fn....
        otherFn()
...
//累加收到的数据个数
private fun accFn(cx: Context): Observable<OnStream> =
    cx.connection.register("acc").doOnNext { (header, bufs, bufPuller) ->
        //验证请求
        println("onHeader:${header.fn}")
        val remoteAppName = cx.connection.remoteInfo.appName
        cx.getIdlestConnectionByAppName(remoteAppName).subscribe {
            println("IdlestConnection('$remoteAppName'):${it.javaClass}")
        }
...
//            .doOnNext { buf -> println("handle $buf on ${Thread.currentThread().name} : ${Thread.currentThread().id}") }
        ...
        //创建下游流（会发送header）
        val downStream = cx.connection.stream(
            Header.newBuilder()
